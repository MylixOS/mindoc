...
as t group by book_id) as team 
			...
			WHERE book.item_id = ? AND (book.privately_owned = 0 or rel.role_id >= 0 or team.role_id >= 0) 
			ORDER BY order_index desc,book.book_id DESC  ` + sqltil.DBSpecificLimitOffset(o.Driver().Type())
		...
		sql := `SELECT book.*,rel.*,member.account AS create_name FROM md_books AS book
			...
			WHERE book.item_id = ? AND book.privately_owned = 0 ORDER BY order_index desc,book.book_id DESC  ` + sqltil.DBSpecificLimitOffset(o.Driver().Type())
